...
interface Stats { total: number; byTier: Record<string, number>; totalOrders: number; pendingOrders: number; totalRevenue: number; }
interface RecentOrder { id: string; orderNumber: string; status: string; tierAtPurchase: string; total: number; createdAt: string; user?: { nombre: string; apellido: string }; }
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      request('GET', '/api/admin/orders?pageSize=10'),
    ]).then(([s, o]) => {
        ...
        <div className="grid grid-cols-3 gap-4">
          {Object.entries(stats?.byTier ?? {}).map(([tier, count]) => (
            <div key={tier} className="text-center p-4 rounded-lg bg-ivory">
              <span className={`inline-block text-[10px] font-semibold uppercase tracking-wider px-3 py-1 rounded-full ${TIER_COLORS[tier] || ''}`}>{TIER_LABELS[tier] || tier}</span>
              <p className="text-2xl font-bold text-stone-800 mt-2">{count as number}</p>
            </div>
          ))}
        </div>
      </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[10px] text-stone-400 uppercase tracking-wider border-b border-stone-100">
                  ...
                  <th className="text-left px-6 py-3 font-medium">Tier</th><th className="text-right px-6 py-3 font-medium">Total</th>
                  <th className="text-left px-6 py-3 font-medium">Estado</th><th className="text-left px-6 py-3 font-medium">Fecha</th>
                </tr>
              </thead>
              <tbody>
