...
import { TIER_MAP_REVERSE } from '../../services/api'
...
export default function AdminUsuarios() {
  ...
  function loadUsers() {
    setLoading(true)
    admin.getUsers({ page, pageSize: 10, search: search || undefined, tier: tierFilter || undefined })
  ...
  async function changeTier(userId, newTier) {
    ...
    try {
      await admin.changeTier(userId, newTier)
  ...
  return (
    <div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="text-[10px] text-stone-400 uppercase tracking-wider border-b border-stone-100">
                  ...
                  <th className="text-right px-6 py-3 font-medium">Compras</th>
                ...
                {users.map((u, i) => (
                  <tr key={u.id} className={`${i % 2 === 0 ? 'bg-white' : 'bg-ivory/50'} hover:bg-stone-50 transition-colors`}>
                    ...
                    <td className="px-6 py-3.5 text-right font-medium text-stone-700" style={{ fontVariantNumeric: 'tabular-nums' }}>
                      {formatCOP(u.totalComprasAcumulado)}
                    </td>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-t border-stone-100">
            ...
            <div className="flex items-center gap-2">
              <button onClick={() => setPage(Math.max(1, page - 1))} disabled={page === 1}
                className="p-2 border border-stone-200 rounded disabled:opacity-30 cursor-pointer hover:bg-stone-50">
                <ChevronLeft size={14} />
              </button>
      ...
      {(selectedUser || detailLoading) && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setSelectedUser(null)} onKeyDown={(e) => e.key === 'Escape' && setSelectedUser(null)}>
            ...
            {detailLoading ? (
              ...
              <div className="p-6">
                ...
                <div className="grid grid-cols-2 gap-4 text-sm mb-6">
                  <div><span className="text-[10px] text-stone-400 uppercase tracking-wider block">Tier</span>
                    <span className={`inline-block mt-1 text-[10px] font-semibold uppercase tracking-wider px-2 py-0.5 rounded-full ${TIER_COLORS[selectedUser.tier] || ''}`}>{TIER_LABELS[selectedUser.tier]}</span>
                  </div>
